...
type MonthListProps = {
    ...
    handleClickMonth: (year_id: number, time_id: number, month: number) => void;
        ...
        <div className="months-btn-area">
        {
            times && times.length > 0 ? times.map((value):any => (
                <div className="month-btn-body" key={value.time_id}>
                    <button className="dropdown-btn btn-month" onClick={() => handleClickMonth(value.year_id, value.time_id, value.month)}>{value.month}</button>
